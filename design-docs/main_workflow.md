...
Before any stage runs, shift settings are re-validated against the tolerance constraint in `config.md` — each tolerance must be at most half the zone interval. The settings screen already blocks violating edits, so this is defence in depth against settings altered outside that flow. A violation aborts generation with an Arabic error before any work is done.

See [[screen_report_generate.md]] and [[config.md]].
...
### Stage 4 — Schedule Detection and Shift Period Construction
...
Shift periods are built during this stage, not by a later one. The zone layout a period is tested against is the same layout the overtime calculator later reads, so the periods that survive detection carry their zone results straight through. This is why there is no separate shift period extraction stage.

Detection judges zone satisfaction with a wider edge tolerance than overtime validity does — classification asks whether someone works shifts, which a punch well off the mark still answers yes to. Both flags come from the same zones; only the stored one governs overtime. See `config.md` for why the two are kept apart.
...
### Stage 6 — Daily Employees Period Extraction
...
Each period is validated and overtime is calculated. Every check runs on every period — none short-circuits another — and each failure adds its own reason to a set. A period is valid only when that set is empty.

See  `overtime_calculation_shift.md`

### Stage 8 — Daily Employees Overtime Calculation

Validated and overtime is calculated, collecting failure reasons into a set exactly as the shift calculator does.

See `overtime_calculation_daily.md`.

### Stage 9 — Store and Navigate
...
Any failure during Stages 3–9, or the settings check in Stage 2, aborts generation entirely. No partial results are stored. The user sees an Arabic error message and remains on the Report Generation screen with all inputs intact.
...
- The CPU-bound stages run in a background isolate so the UI keeps rendering during generation; this does not change any stage's inputs or outputs
- Configuration is validated before the pipeline starts, never mid-run — a settings problem fails fast in Stage 2 rather than surfacing as a malformed result later
...
- **Stage 9 writes are atomic — a single SQLite transaction wraps all inserts. Either the full report is stored or nothing is.**
