...
## 20260901-1800 | Shift Tolerance Defaults, Settings Unit Labels & Fullscreen Launch | TASK

**Task:** Four independent changes. (1) Default `shift_edge_tolerance` raised 30→60 minutes and `shift_inner_tolerance` raised 120→179 minutes (both still within the existing `≤ zone_interval/2` constraint) — code defaults only, so it affects fresh installs; the already-running database keeps its seeded values until edited via Settings. (2) Every minute-based Settings field now shows a "دقيقة" unit suffix directly in the field, and every hour-based field a "ساعة" suffix, instead of the unit being discoverable only via the hover tooltip. The shift zone-count read-only field was also relabeled "عدد نقاط التحقق المحتسبة" → "عدد البصمات المطلوبة" and its value box width aligned with the number fields above it. (3) The Windows app now launches filling the monitor's available work area (screen minus taskbar) instead of a small fixed window — borderless, no title bar. (4) Since going borderless removed the native close button, several approaches were tried before landing on the final one: a global top strip (crashed at runtime — Tooltip/showDialog need a Navigator/Overlay ancestor that a widget outside go_router's routed subtree doesn't have), then a working but visually unwanted version of that same strip, and finally a plain red "إغلاق البرنامج" button (solid red, white text, no icon) placed below the login button on the login screen — reachable by any logged-in user via the existing logout button, so closing the app requires logging out first by design.

**Diverged:** `router.md`/`screen_configuration.md` predate the login gate and unit-label UI entirely; not treated as contradictions, just pre-existing drift already flagged in the doc index.
...
## 20260908-1500 | Unified Employee Report Screen — Merged Shift/Daily/Undetected List | TASK

**Task:** Replaced the Report screen's شift/دوام صباحي tabs and the separate "غير محددين" screen with a single unified screen listing all three employee categories together, so searching/filtering only ever happens in one place instead of three. One shared filter surface (name, department, overtime, inclusion) now applies across the merged list, plus a new type filter (checkboxes for the three categories, same paired show/hide convention as the other filters). Each row carries a type indicator; undetected rows reuse the existing selection-column slot to show their failure reason instead of a checkbox, since they have no inclusion concept — mirroring how the overtime column already varies by row type. The four summary cards moved to the screen's footer (the slot the removed tab buttons occupied) and were rescoped to included employees only: headcount, off-day overtime, working-day overtime, and their total — shift overtime is folded into the working-day bucket alongside daily's regular-day overtime, since shift overtime is definitionally working-day overtime with no off-day concept of its own. The row-level overtime display was unified to the same visual format for both types for this same reason. Export was consolidated into one unified file (single sheet, type column, shared columns) but kept the pre-existing rule that export always covers only included/paid employees regardless of any active on-screen filter.

**Diverged:** The standalone undetected-employee export (previously its own full-list export, independent of any filter) no longer exists — undetected employees have no inclusion status, so they never qualify for the unified export's included-only scope. This is a deliberate, user-confirmed trade-off of unification, not an oversight.
...
## 20260922-1400 | Clear-Daily Pre-Pass & Off-Day Threshold Raise | TASK

**Task:** Diagnosed and fixed two detection bugs reported against a real month of attendance data (`4th-month`). (1) Daily employees with three punches a day (entry, midday, exit) were being misclassified as shift: their punches coincidentally satisfied 4 of the 5 shift zones on the shared 6-hour zone grid — the entry and midday/exit punches landing inside zone-center tolerance windows, plus the next day's entry landing in the final zone — crossing the `min_zones_satisfied` bar on nearly every working day. Added a "clear daily" pre-pass to schedule detection: an employee present on ≥62% of non-Friday/Saturday report days is classified daily outright, before the zone/anchor-pair check ever runs. The 62% cutoff was calibrated by measuring the real dataset — legitimate rotating-shift employees (several source departments are guard/safety-shift rosters on a roughly 1-on/1-off cadence) topped out at 59% non-weekend density, while genuine daily employees sat at 64% and above, leaving a clean empty gap the threshold sits in. (2) The off-day density threshold was raised from 25% to 60%: a partial-holiday day in the sample data drew 36.5% attendance — a clear skeleton crew, far below the ~70-76% of a genuine weekday, but still well above the old 25% bar, so it was wrongly classified as a regular working day. Both fixes were validated by simulating the full classification algorithm against the real dataset before and after the change: the two originally reported employees and four other affected employees now classify correctly as daily, the missed holiday is now correctly flagged, and no other day or previously-correct shift employee changed classification.

**Diverged:** None — `config.md`, `schedule_detection.md`, and `off_day_detection.md` were updated to document both changes as the new deliberate design, not left contradicting it.

**Rejected:** A second pre-pass to remove "clear shift" employees (matching a repeating 1-day-on/2-days-off pattern) before running the zone check — deferred, since no evidence exists of the reverse misclassification (shift workers misfiled as daily) in this data or in prior history. An initial 70% candidate for the clear-daily threshold — narrowed to 62% after a density histogram revealed a handful of genuine daily employees sitting at 64-68%, which 70% would have left unresolved. An off-day threshold of 75% (upper end of the user's original suggestion) — rejected in favor of 60%, which has almost no margin loss below the lowest real weekday observed (69.5%) while still comfortably catching the 36.5% holiday case.

---

## 20260922-1700 | Cross-Month Validation & Start-Time Ambiguity Rescue | TASK

**Task:** Two follow-ups after the prior task's fix. (1) Cross-validated that fix against a second, independent real month (January): the same 59%/64% density gap held with different employees, the off-day threshold caught two more genuinely missed holiday days, and the two originally reported employees recurred with the same pattern, now resolved correctly — confirming the fix generalizes rather than being tuned to one month. (2) Diagnosed and fixed a second, unrelated detection bug found by inspecting the live production database directly: two employees with an unambiguous single 24-hour shift pattern were being marked undetected ("وقت بداية المناوبة غير واضح") even though they clearly cleared the minimum-valid-periods bar. Root cause: the live app has two configured shift start times (08:00 and 23:00), and an employee whose real "on" day carries many punches spread from morning to night can coincidentally satisfy most zones under the *wrong* configured start time too — not a real competing shift pattern, but the same real shift days re-scored under a different clock offset. This pushed the winning candidate's raw-count margin over the runner-up below the required threshold, so a correctly-detected shift employee was discarded entirely rather than merely mislabeled. Measured as non-rare: 13 employees in April and 19 in January hit this exact failure mode, with several recurring across both months. Fixed with a narrowly-scoped, strictly-additive safety net that runs only at the moment the existing start-time-confidence check is about to reject an employee: it accepts the rejection's own original winner anyway if the runner-up candidate can't independently clear the same minimum-valid-periods bar on days that don't overlap the winner's — reusing the existing threshold rather than adding a new one. Because it can only ever convert a rejection into an acceptance of the same winner already selected, and never re-examines an employee the existing checks already accepted, it cannot regress any currently-correct classification. Validated by simulating the exact algorithm against both real months: all 13 and all 19 previously-wrong-undetected employees now resolve correctly, with zero regressions and zero start-time reassignments.

**Diverged:** None — `schedule_detection.md` and `config.md` were updated to document the rescue step and its validation as the new deliberate design.

**Rejected:** Scoring start-time candidates by opening-punch consistency (how often a candidate's own configured time has a genuine nearby punch) instead of period-date overlap — tested at scale and rejected, since some genuine shift employees don't reliably punch exactly at open (they clear the zone check via inner zones and the closing punch instead), making that signal indistinguishable from noise for less disciplined punchers; it broke 12 April and 5 January previously-correct classifications in testing. Replacing the existing start-time-confidence check outright with the new period-date-overlap logic, rather than adding it as an additive rescue invoked only on rejection — would have produced identical results on the data measured, but the rescue placement was chosen because it can only ever turn a rejection into an acceptance and can never introduce a new failure mode on cases the existing check already accepts, a strictly safer guarantee than the data alone could confirm.

---

## 20260922-1900 | Start-Time Rescue — Correct Label Selection | TASK

**Task:** Fixed a real defect in the prior task's rescue mechanism, found by the user inspecting its output directly: the rescue correctly turned a wrongful "undetected" outcome into "shift," but kept whichever configured start time the original (bug-affected) raw-count comparison had picked — which was the wrong one for the reported employees, silently mislabeling a real 08:00 shift as 23:00. A wrong label is worse than "undetected" was, since downstream overtime validity is computed against the labelled time's own narrower window, so a mislabeled employee could show near-zero overtime with no flag prompting a manual check.

A first fix (scoring each candidate by how often its own periods contain a genuine punch near its own configured time, then relabeling to whichever scores highest) was implemented, then disproven by a case the user raised: several employees in a control-room department run an authentic 23:00-anchored 24-hour shift whose own routine includes a real checkpoint near 08:00 the next morning — not spillover, but a genuine part of that one shift. That employee's punches carry real, non-spillover evidence near both configured times, so "most opening evidence wins" picked the wrong one.

The user proposed a simpler alternative and asked for a direct comparison: for each day with activity where the *next* day also has activity, take the day's earliest timestamp and vote for whichever configured start time it falls within a fixed window of. Tested head-to-head against the failed case, this call structurally excludes closing/checkpoint days from voting at all — a closing day is by definition followed by a rest day, so it never qualifies to vote — which resolves the exact case the opening-evidence scorer got wrong. The final design keeps the previous task's independence check (deciding whether an employee has one real pattern worth relabeling, or two genuinely competing ones that should stay undetected) and replaces only the relabeling step with this day-pair vote. Re-validated against both real months with zero regressions and the resulting labels confirmed correct by manual inspection of every relabelled employee's raw punch data, including the control-room night-shift cases.

**Diverged:** None — `schedule_detection.md` and `config.md` updated to describe the corrected two-step rescue and why the two earlier relabeling attempts (raw count, then opening evidence) each failed.
...
## 20260924-1600 | Attendance Datetime Text-Format Support & Silent-Drop Warning | TASK

**Task:** Diagnosed a user report of a specific employee (and, it turned out, an entire attendance file) missing from a generated report with no error anywhere — not shift, not daily, not even undetected. Root cause: that file stored its datetime column as plain Arabic 12-hour text (`DD/MM/YYYY HH:MM:SS ص|م`) rather than a native Excel date cell, a shape the pipeline's datetime parser had never recognized. Every row in the file silently failed to parse and was dropped with zero trace, while the file itself still passed upload validation because validation only checked the datetime cell was non-empty text, never that it was actually parseable. Fixed with a single parser shared between file validation and dictionary build, recognizing native Excel date cells, ISO text, and this Arabic text format as a third fallback — validated against the full reported file (13,530/13,530 rows) and two other real files with no regression on the existing native-cell path. File validation was tightened to use the same parser, so a file whose datetime format nothing recognizes now fails loudly at upload with the existing template-mismatch message instead of silently vanishing at generation. As a follow-up, generation now also tracks, per file, how many rows had non-empty but still-unparseable datetime text, and surfaces this as a dismissible, non-blocking warning banner (file name + skipped-row count) on the Report screen immediately after a successful generation — shown once for that report only, never on later revisits.
